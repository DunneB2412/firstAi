...
class AllDotSearchRepresentation(SearchRepresentation[AllDotSearchState]):
    ...
    def move_list(self, vector, unwanted_moves):
        ...
        for move in util.Move.no_stop:
            ...
            if not self.walls[test_vector]:
                moves.append(move)
        for move in unwanted_moves:
            if move in moves:
                moves.remove(move)
        return moves
    ...
    def successors(self, state):
        successors = []
        start_moves = self.move_list(state.vector, [])
        for move in start_moves:
            new_state = state.vector
            moves = []
            new_dots = state.dots
            while len(self.move_list(new_state, [move.opposite])) == 1 and not self.walls[
                new_state + move.vector] or len(moves) == 0:
                moves.append(move)
                new_state = new_state + move.vector
                new_dots = new_dots - {new_state}

            successor = AllDotSearchState(new_state, new_dots)
            successors.append((successor, moves, len(moves)))
        return successors

    """ other way naturally speeds things up
